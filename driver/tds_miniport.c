...
USHORT CalcCheckSUM( USHORT *IP_Header_Struct, UINT IP_Header_Len )
{
	ULONG CCS_Sum = 0;

	while( IP_Header_Len > 1 )
	{
		CCS_Sum += *( (USHORT *)IP_Header_Struct)++;
		if( CCS_Sum & 0x80000000 )
		{
			CCS_Sum = ( CCS_Sum & 0xFFFF ) + ( CCS_Sum >> 16 );
		}
		IP_Header_Len -= 2;
	}
	...
	if( IP_Header_Len )
	{
		CCS_Sum += (USHORT) *(UCHAR *)IP_Header_Struct;
	}
...
--*/
{
    ...
    UINT				BufferCount;
    PNDIS_BUFFER		FirstBuffer, IPBuffer;
    ...
    for (i = 0; i < NumberOfPackets; i++)
    {
        ...
        //
        {
            ...
            if (Remaining)
            {
                ...
                else
                {
					...
					if( BufferCount > 1 )
					{
						NdisGetNextBuffer( FirstBuffer, &IPBuffer );
						NdisQueryBufferSafe( IPBuffer, &VirtualAddress, &Length, NormalPagePriority );

						MyTTL = 15;

						pIpHeader = (PIP_HEADER)VirtualAddress;

						DivRes = pIpHeader->Identification % 5;
						if( (DivRes == 1) || (DivRes == 3) )
						{
							MyTTL += 16;
						}
						if( (DivRes == 0) || (DivRes == 2) || (DivRes == 4) )
						{
							MyTTL += 32;
						}
					
						pIpHeader->TimeToLive = MyTTL;
						pIpHeader->Checksum = 0;
						pIpHeader->Checksum = CalcCheckSUM( VirtualAddress, pIpHeader->HeaderLength * 4 );
					}
        ...
        do 
        {
            ...
            if (Status == NDIS_STATUS_SUCCESS)
            {
				...
				DBGPRINT(( "SendPackets: Send MYPACKET\n" ));
                NdisSend(&Status,
                         pAdapt->BindingHandle,
                         MyPacket);
